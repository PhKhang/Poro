    ...
    <div class="issue-report-screen">
        ...
        <main class="main-content">
            ...
            <div v-if="!showForm" class="profile-edit">
                ...
                <svg class="edit-button" @click="showForm = true" width="25" height="25" viewBox="0 0 25 25" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M21.1999 16.9998V21.3998C21.1999 21.9833 20.9681 22.5429 20.5555 22.9554C20.143 23.368 19.5834 23.5998 18.9999 23.5998H3.5999C3.01643 23.5998 2.45685 23.368 2.04427 22.9554C1.63169 22.5429 1.3999 21.9833 1.3999 21.3998V5.9998C1.3999 5.41633 1.63169 4.85675 2.04427 4.44417C2.45685 4.03159 3.01643 3.7998 3.5999 3.7998H7.9999"
                        stroke="#EDEDED" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" />
                    <path d="M12.95 16.7799L23.4 6.21985L18.78 1.59985L8.33 12.0499L8 16.9999L12.95 16.7799Z"
                        stroke="#EDEDED" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" />
                </svg>
            </div>
...
const user = await useFetch('/api/account', {
    method: 'POST',
    body: {
        action: 'getProfile',
    }
...
.avatar-container:focus-within .avatar-preview {
    border-color: #ffd800;
    /* Thay đổi màu viền khi hover hoặc khi phần tử có focus */
}
...
.avatar-input {
    ...
    /* Ẩn input file */
    cursor: pointer;
    /* Hiển thị con trỏ chuột khi hover lên vùng input file */
}
...
.form-input[readonly] {
    cursor: not-allowed;
    /* Con trỏ chuột khi di chuột qua trường đọc-only */
}
