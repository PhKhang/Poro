        ...
        <div class="close-btn" @click="hideElement">
          <svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0_1_1163)">
              <path
                d="M13.7328 5.7334H3.06607C1.59541 5.7334 0.399414 6.9294 0.399414 8.40005C0.399414 9.87071 1.59541 11.0667 3.06607 11.0667H13.7327C15.2034 11.0667 16.3994 9.87071 16.3994 8.40005C16.3994 6.9294 15.2034 5.7334 13.7328 5.7334Z"
                fill="#EDEDED" stroke="#EDEDED" stroke-width="0.03125" />
            </g>
            <defs>
        ...
        <div class="youtube-header">
          <span class="youtube-icon">
            <svg width="25" height="18" viewBox="0 0 25 18" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M24.2698 3.39445C23.9896 2.35921 23.1809 1.55052 22.1457 1.27038C20.2833 0.762451 12.7875 0.762451 12.7875 0.762451C12.7875 0.762451 5.29164 0.777843 3.42923 1.28577C2.39398 1.56591 1.58529 2.3746 1.30516 3.40985C0.741817 6.71909 0.523253 11.7615 1.32055 14.9383C1.60068 15.9736 2.40937 16.7823 3.44462 17.0624C5.30703 17.5703 12.8029 17.5703 12.8029 17.5703C12.8029 17.5703 20.2987 17.5703 22.1611 17.0624C23.1963 16.7823 24.005 15.9736 24.2852 14.9383C24.8793 11.6245 25.0624 6.58518 24.2698 3.39445Z"
                fill="#FF0000" />
...
const fetchThemes = await useFetch('/api/backgroundhelper',
  {
    method: 'POST',
    body: {
      action: 'getThemeList',
...
console.log('Theme:', fetchThemes.data.value);
themesJs = fetchThemes.data.value;


const themes = themesJs.map(theme => ({ id: theme.id, icon: theme.icon, name: theme.name }));

function updateTemporaryURL(event) {
  youtubeURL.value = temporaryYoutubeURL.value;
  temporaryYoutubeURL.value = '';
}
...
.youtube-url-container textarea {
  ...
  outline: none;
  resize: none;
  /* Ngăn không cho thay đổi kích thước */
  overflow: hidden;
  /* Ẩn văn bản tràn */
  height: 100%;
  /* Đặt chiều cao để phù hợp với container */
  line-height: 41px;
  /* Đặt line-height bằng chiều cao để căn giữa */
  white-space: nowrap;
  color: #ededed;
  font-family: "Source Sans 3";
  font-size: 16px;
